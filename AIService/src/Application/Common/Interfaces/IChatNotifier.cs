namespace AIService.Application.Common.Interfaces
{
    public interface IChatNotifier
    {
        ...
        Task SendMessageChunkAsync(string userId, Guid messageId, string chunk);

        // 2. Báo hiệu AI đã trả lời xong một câu
        // Để Frontend biết mà tắt cái animation "AI is typing..."
        Task SendMessageCompletedAsync(string userId, Guid messageId);

        // 3. Nhận toàn bộ tin nhắn 1 lần (Dùng cho tin nhắn hệ thống hoặc chat thông thường không cần streaming)
        Task SendMessageAsync(string userId, Guid messageId, string role, string content);
