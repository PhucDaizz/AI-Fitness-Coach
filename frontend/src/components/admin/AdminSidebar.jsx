  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-[#0e0e0e]/90 backdrop-blur-xl flex justify-around items-center py-4 z-[100] border-t border-white/5">
        <Link
          to="/admin"
          className={`flex flex-col items-center ${isActivePath('/admin') ? 'text-primary' : 'text-on-surface-variant'}`}
        >
        ...
        <Link
          to="/admin/exercises"
          className={`flex flex-col items-center ${isActivePath('/admin/exercises') ? 'text-primary' : 'text-on-surface-variant'}`}
        >
          <span className="material-symbols-outlined">fitness_center</span>
          <span className="text-[8px] font-bold uppercase tracking-tighter mt-1">Exercises</span>
        </Link>
        ...
        <Link
          to="/admin/users"
          className={`flex flex-col items-center ${isActivePath('/admin/users') ? 'text-primary' : 'text-on-surface-variant'}`}
        >
        ...
        <Link
          ...
          className={`flex flex-col items-center ${isActivePath('/admin/maintenance') ? 'text-primary' : 'text-on-surface-variant'}`}
        >
          <span className="material-symbols-outlined">settings</span>
          <span className="text-[8px] font-bold uppercase tracking-tighter mt-1">Tools</span>
        </Link>
      </nav>
      ...
      >
        ...
        <div
          className="absolute inset-0 bg-[#000000]/80 backdrop-blur-2xl"
          onClick={() => setIsOpen(false)}
        />

        {/* Mobile Menu Content */}
        <div
          className={`absolute inset-x-0 bottom-0 top-20 bg-surface-container rounded-t-[3rem] border-t border-white/10 p-8 transform transition-transform duration-500 ease-out flex flex-col ${
            ...
            <div className="mt-8 space-y-4 pt-8 border-t border-white/5">
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-4 rounded-2xl bg-white/5 text-[#adaaaa] text-[10px] font-black uppercase tracking-widest"
                >
      ...
      <aside
        className={`hidden md:flex h-screen w-64 fixed left-0 top-0 bg-[#000000] flex-col py-8 z-[90] border-r border-white/5`}
      >
        <div className="px-6 mb-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-['Inter'] uppercase tracking-widest text-[0.6rem] font-bold text-on-surface-variant">
          ...
          {navItems.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-['Inter'] uppercase tracking-widest text-[0.75rem] font-bold">
                  {item.label}
                </span>
              </Link>
            );
        ...
        <div className="px-6 mt-auto space-y-4 pt-6 border-t border-white/5">
          <div className="space-y-2">
            <Link
              to="#"
              className="flex items-center gap-3 text-[#adaaaa] text-[0.75rem] font-bold uppercase tracking-widest hover:text-white"
            >
              <span className="material-symbols-outlined text-sm">help</span> Support
            </Link>
            <button
              ...
              className="w-full flex items-center gap-3 text-[#adaaaa] text-[0.75rem] font-bold uppercase tracking-widest hover:text-white"
            >
              <span className="material-symbols-outlined text-sm">logout</span> Log Out
            </button>
          </div>
