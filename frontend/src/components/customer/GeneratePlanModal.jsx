...
const GeneratePlanModal = ({ isOpen, onClose, onGenerate, onAbort, isGenerating }) => {
  ...
  React.useEffect(() => {
    ...
    const stepMs =
      lastIndex > 0 ? FINAL_STATUS_MESSAGE_DELAY_MS / lastIndex : FINAL_STATUS_MESSAGE_DELAY_MS;

    const interval = setInterval(() => {
      const elapsedMs = Date.now() - startedAt;
      const nextIndex = Math.min(Math.floor(elapsedMs / stepMs), lastIndex);

      setStatusIndex(nextIndex);
    }, STATUS_MESSAGE_TICK_MS);
        ...
        {!successData ? (
          <>
            <header className="mb-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-sm font-black uppercase tracking-[0.2em] text-primary">
                  {t('workout_plans.modal.generator_title')}
                </h2>
              </div>
              <h3 className="text-3xl font-black text-white italic uppercase tracking-tighter leading-none">
                {t('workout_plans.modal.title')}{' '}
                <span className="text-primary">{t('workout_plans.modal.title_highlight')}</span>
              </h3>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant ml-2">
                  {t('workout_plans.modal.duration')}
                </label>
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant ml-2">
                  {t('workout_plans.modal.start_date')}
                </label>
                <input
                  type="date"
                  value={startsAt}
          ...
          <div className="text-center py-4 animate-in slide-in-from-bottom duration-500">
            ...
            <h3 className="text-3xl font-black text-white italic uppercase tracking-tighter mb-4">
              {t('workout_plans.modal.success_title')}{' '}
              <span className="text-primary">
                {t('workout_plans.modal.success_title_highlight')}
              </span>
            </h3>
